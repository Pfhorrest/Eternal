...
function Triggers.platform_activated(plat)
	if plat.media then -- media are used to link platforms & associated platforms
		if plat.media.index > 5 then -- forcefield got deactivated
			for poly in Polygons() do
				if poly.media then
					if poly.media.index == plat.media.index and poly.ceiling.transfer_mode == "landscape" then
						poly.ceiling.texture_index = 1 -- change landscape to match
					end
				end
			end
			calculate_triangles() -- rebuild list of polygons that can have rain
		end
	end
end
